...
@:pure 
abstract Mapping<K, V>(PureList<MapEntry<K, V>>) from PureList<MapEntry<K, V>> to PureList<MapEntry<K, V>> {
  ...
  **/
  ...
  @:extern inline public function iterator()
    return getCondensed().or(alloc).touch().iterator();

  /**
   *  Returns an Iterator over the keys of `this` Mapping.
   */
  @:extern inline public function keys()
    return getCondensed().or(alloc).touch().keys();

  @:to 
  @:extern 
  inline public function condensed():Mapping<K, V> {
  ...
  @:from @:extern inline static function ofMutable<K, V>(v:Map<K, V>):Mapping<K, V> {
    var ret = new PureList<MapEntry<K, V>>();
