...
class DoubleLinkedList<T> {
    ...
    static function dissolveSlice<T>(list:DoubleLinkedList<T>, from_:DoubleLinkedNode<T>, ?to:DoubleLinkedNode<T>) {
        if (list.isEmpty) return ;
        if (to == null) to = list.tail.prev;
        if (inline same(list.head, from_)) from_ = list.head.next;
        if (inline same(list.tail, to)) to = list.tail.prev;
        if (inline same(list.tail, from_))
            throw new pm.Error('Invalid slice nodes provided', 'InvalidArgument');
        var node = from_;
        while (!same(node, to)) {
            var fwd = node.next;
    ...
    public static inline function clear<T>(list: DoubleLinkedList<T>) {
        while (!list.isEmpty)
            removeFirst(list);
    }
    ...
    public inline function consume() return new DoubleLinkedListConsumer<T>(this);
...
class DoubleLinkedListConsumer<T> {
    public var list:DoubleLinkedList<T>;
    public var node:DoubleLinkedNode<T>;
    public inline function new(l:DoubleLinkedList<T>) {
        list = l;
        node = list.head;
    }

    /**
      @returns whether the next node is a data-node or the terminating node
     **/
	public inline function hasNext():Bool return list.head.next != list.tail;

	/**
      @returns the value attached to the popped node
     **/
    @:extern
	public inline function curr():T {
        return 
            if (this!=null && this.node!=null) this.node.data
            else null; 
    }
	 ...
	 */
	public inline function next():T {
        node = DoubleLinkedList.removeFirst(list);
        return node.data;
	}
}
