...
abstract DTrigger<R, E> (IDTrigger<R, E>) from IDTrigger<R, E> {
    public function new(?init:Dynamic) {
        this = _instantiate(init);
    }

/* === [Methods] === */

    public inline function listen(listener: Callback<Outcome<R, E>>):CallbackLink {
        return this.listen(listener);
    }

    public inline function trigger(outcome: Outcome<R, E>) {
        this.trigger(outcome);
    }

/*
 [TODO] make `resolve`, `reject` interface methods
*/
    public inline function resolve(result: R):R {
        trigger(Success(result));
        return result;
    }

    public inline function reject(error: E) {
        trigger(Failure(error));
    }

/* === [Statics] === */

    static var _useFallbackAnyway:Bool = false;

    /**
      @param init 
        if `init` is an object and `init.useNativePromise == false`, or `init.useFallbackAnyway == true` then the fallback (non-platform-optimized) class is used
        otherwise, so far only js has a platform-specific implementation @see `JsPromiseDTrigger<R, E>`

      @returns `IDTrigger<R, E>` instance
    **/
    static function _instantiate<R, E>(init: Null<Dynamic> = null):IDTrigger<R, E> {
        /**
          [TODO] create further flexibility via `init`
         **/
        #if js
            var useNativePromise = true;
            if (_useFallbackAnyway)
                useNativePromise = false;
            else if (init != null && init.useNativePromise == false) {
                useNativePromise = false;
            }
            else if (js.Syntax.code('!({0} in {1})', 'Promise', js.Lib.global)) {
                useNativePromise = false;
                _useFallbackAnyway = true;
                trace('will use fallback async implementation');
            }
            if (useNativePromise) {
                return new JsPromiseDTrigger();
            }
        #end

        return new FutureDTrigger();
    }
}

class FutureDTrigger<R, E> implements IDTrigger<R, E> {
    public var future:FutureTrigger<Outcome<R, E>>;
    public var trigger:Outcome<R, E> -> Bool;
    public var listen:Callback<Outcome<R, E>> -> CallbackLink;

    public inline function new(#if debug ?pos:haxe.PosInfos #end) {
        future = new FutureTrigger();
        listen = function(cb) {
            return future.handle(cb);
        };
        trigger = function(o) {
            // var isCallSuperfluous = !future.trigger(o);
            // var position = #if debug pos #else null #end;
            // pm.Assert.assert(!isCallSuperfluous, new pm.Error('Cannot call trigger more than once', position), position);
            return future.trigger(o);
        };
    }
}

#if js
class JsPromiseDTrigger<R, E> implements IDTrigger<R, E> {
	public var trigger:Outcome<R, E>->Bool;
	public var listen:Callback<Outcome<R, E>>->CallbackLink;
    public var promise:{promise:JsPromise<Outcome<R, E>>, resolve:(value:Outcome<R, E>)->Void, reject:(error:Dynamic)->Void};
    public function new() {
        var res, rej, hasBeenCalled = false;
        var prom = new JsPromise<Outcome<R, E>>(function(resolve, reject) {
            res = resolve;
            rej = reject;
        });
        promise = {
        ...
        trigger = function(o) {
            if (!hasBeenCalled) {
                /***
                [NOTE] this is probably how this should work...
                    `switch o {
                        case Success(result):
            ...
            throw new pm.Error('Should be unreachable');
        };

        inline function rebind_listen() {
            listen = function(cb):CallbackLink {
                cb(_outcome);
                #if debug
                return function() {
                    throw new pm.Error('attempted to cancel a callback registered to a resolved Future');
                };
                #else
                return untyped null;
                #end
        ...
        listen = function(cb: Callback<Outcome<R, E>>) {
            if (_outcome != null) {
                rebind_listen();
                // cb(_outcome);
                return listen(cb);
			...
			var cancel:CallbackLink = function() {
				cancelled = true;
			};

            promise.promise.then(function(outcome: Outcome<R, E>) {
    ...
    var _outcome:Null<Outcome<R, E>> = null;
}
#end

interface IDTrigger<R, E> {
    var trigger: (outcome: Outcome<R, E>) -> Bool;
    var listen: (listener: Callback<Outcome<R, E>>) -> CallbackLink;
}
...
abstract Future<T>(FutureObject<T>) from FutureObject<T> to FutureObject<T> {
     ...
     */
    static public inline function flatten<A>(f: Future<Future<A>>):Future<A> {
     ...
     */
    @:from 
    static public function ofJsPromise<A>(promise:JsPromise<A>):Surprise<A, Error> {
        return Future.async(function(cb) {
            promise
            .then(function(a) cb(Success(a)))
            .catchError(function(e: JsError) {
                cb(Failure(cast new pm.Error.ValueError(e, e.message)));
            });
        });
    }
    #end
    ...
    @:from 
    static inline function ofAny<T>(v:T):Future<T> {
  ...
  static public function ofMany<A>(futures:Array<Future<A>>, ?gather:Bool = true) {
    ...
    @:from 
    static function fromMany<A>(futures: Array<Future<A>>):Future<Array<A>> {
    ...
    @:noUsing 
    public static function async<A>(f: (A -> Void) -> Void, ?lazy = false):Future<A> {
        if (lazy) {
    ...
    @:noCompletion 
    @:op(a || b) 
    static public function or<A>(a:Future<A>, b:Future<A>):Future<A> {
        return a.first(b);
    }
    ...
    @:noCompletion 
    @:op(a || b) 
    static public function either<A, B>(a:Future<A>, b:Future<B>):Future<Either<A, B>> {
    ...
    @:noCompletion 
    @:op(a && b) 
    static public function and<A, B>(a:Future<A>, b:Future<B>):Future<Pair<A, B>> {
        return a.merge(b, function (a, b) return new Pair(a, b));
    }
    
    @:noCompletion 
    @:op(a >> b) 
    static public function _tryFailingFlatMap<D, F, R>(f:Surprise<D, F>, map:D -> Surprise<R, F>) {
    ...
    @:noCompletion 
    @:op(a >> b) 
    static public function _tryFlatMap<D, F, R>(f:Surprise<D, F>, map:D->Future<R>):Surprise<R, F> {
        return f.flatMap(function (o) {
            return switch o {
    ...
    @:noCompletion 
    @:op(a >> b) 
    static public function _tryFailingMap<D, F, R>(f:Surprise<D, F>, map:D->Outcome<R, F>) {
        return f.map(function (o) return o.flatMap(map));
    }

    @:noCompletion 
    @:op(a >> b) 
    static public function _tryMap<D, F, R>(f:Surprise<D, F>, map:D->R) {
    ...
    @:noCompletion 
    @:op(a >> b) 
    static public function _flatMap<T, R>(f:Future<T>, map:T->Future<R>) {
        return f.flatMap(map);
    }

    @:noCompletion 
    @:op(a >> b) 
    static public function _map<T, R>(f:Future<T>, map:T->R) {
        return f.map(map);
    }
