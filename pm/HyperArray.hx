 ...
 * These dimensions can be specified at runtime.
 *
 * Usage:
...
class HyperArray<T> {
	 ...
	 */
	public function index(coords: Coords):Index {
	 ...
	 */
	public function coords(index:Index):Coords {
		return [for (i in 0...dimensions) Std.int(index / strides[i]) % size(i)];
	}

	public function get(coords: Coords):T {
		return data[inline index(coords)];
	}

	public function set(coords:Coords, value:T):T {
		return data[inline index(coords)] = value;
	}
	...
	public inline function swap(a:Coords, b:Coords):Void {
		swapAt(inline index(a), inline index(b));
	}
	...
	public function region(?lbound:Coords, ?ubound:Coords):Iterator<Coords> {
		if (lbound == null)
			lbound = [for (i in 0...dimensions) 0];
		if (ubound == null)
			ubound = [for (i in 0...dimensions) size(i)];
