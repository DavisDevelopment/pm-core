...
class Bigs {
  ...
  public static function smallToArray(n : Int) : Array<Int> {
    assert(n >= 0, 'Bigs.smallToArray should always be non-negative: $n');
    if(n < BASE)
      return [n];
    if(n < DOUBLE_BASE)
  ...
  public static function toInt64(value : BigIntImpl) : haxe.Int64 {
    return pm.Int64s.parse(value.toString());
  }
  ...
  public static function parseBase(text : String, base : Int) : BigIntImpl {
    ...
    if(length <= LOG_MAX_INT / Math.log(base))
      return new Small(Base.fromBase(text, base) * (isNegative ? -1 : 1));
