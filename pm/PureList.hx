...
abstract PureList<T>(Node<T>) from Node<T> {
  ...
  public function concat(that:PureList<T>):PureList<T>
    return
      if (this == null)
        that;
      else if (that == null)
  ...
  public function append(value:T):PureList<T>
    return
      if (this == null)
        new Node(1, value);
      else
  ...
  public function prepend(value:T):PureList<T>
    return
      if (this == null)
        new Node(1, value);
      else
  ...
  public function replace(select:ReplaceSelector<T>, generate:ReplaceGenerator<T>):PureList<T>
  ...
  public function filter(f:T->FilterResult):PureList<T>
    return
      if (this == null) null;
      else this.filter(f);

  public function map<A>(f:T->A):PureList<A>
    return fromArray([for(i in iterator()) f(i)]);

  public function select<A>(f:T->haxe.ds.Option<A>):PureList<A> {
    var arr = [];
  ...
  static public inline function single<A>(v:A):PureList<A>
    return new Node(1, v);

  public function toArray():Array<T>
    return [for(v in iterator()) v];

  @:from static public function fromArray<A>(i:Array<A>):PureList<A> {
    var ret = null,
        len = 0,
        pos = i.length;
  ...
  @:from static function ofRepresentation<T>(rep:tink.json.Representation<Array<T>>):PureList<T>
    return PureList.fromArray(rep.get());
...
@:generic private class Node<T> {
  ...
  public function filter(f:T->FilterResult):Node<T> {
    ...
    return @:privateAccess PureList.fromArray(ret).node();
  }
