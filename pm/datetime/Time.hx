...
abstract Time(Int64) {
    ...
    **/
    ...
    public static function fromString(s:String):Time {
		...
		if (!pattern.match(s))
			throw new pm.Error('unable to parse Time string: "$s"');
	...
	@:op(A + B) 
    inline public function add(that:Time)
		return new Time(ticks + that.ticks);

	@:op(A + B) 
    inline public function addTicks(that:Int64)
		return new Time(ticks + that);

	@:op(A - B) 
    inline public function subtract(that:Time)
		return new Time(ticks - that.ticks);
