...
abstract BitSet(Array<Int32>) from Array<Int32> {
	...
	**/
	public inline function new(length: Int) {
		// store the BitSet length at block index 0
		this = [length]; 
		
        #if !js 
            // resizing is not needed in JS but might still be a good idea
	...
	**/
	...
	public static inline function fromString(str:String):BitSet {
		var chars = str.split("");
        return chars.reducei(function(acc:BitSet, char, i) {
            acc.setAt(i, char=='1');
            return acc;
        }, new BitSet(0));
	}
	...
	**/
	...
	public function at(index:Int):Bool {
        ...
        final blockIndex = ((index / blockSize)+1).floor();
		final block = this[blockIndex];
		final bitIndex = index % blockSize;
	...
	**/
	@:arrayAccess
	public inline function setAt(index:Int, value:Bool):Bool {
		final blockIndex:Int = ((index / blockSize) + 1).floor();

		if (blockIndex >= this.length) {
			// Arrays.resize(this, blockIndex + 1, 0);
            this.resize(blockIndex + 1);
		}
	...
	**/
    ...
    public inline function quickDirtyClone():BitSet {
        return (this.copy() : BitSet);
    }
	...
	**/
    @:deprecated('TODO: unfuck implementation')
	public function expand(count:Int):BitSet {
        return fromBools(
            range(length).flatMap(function(index: Int) {
                var arr = Arrays.alloc(count+1);
                arr.fill(at(index));
                return arr;
            })
        );
	...
	**/
    ...
    public static inline function ceq(a:BitSet, b:BitSet):Bool {
        var inequalityFound = false;
        inequalityFound = a.length != b.length;
        if (!inequalityFound) for (i in 0...a.length) {
            if (a.at(i) != b.at(i)) {
    ...
    /**
      ...what in the fuck?
     **/
	function stupid_combine(right:BitSet, combiner:Bool->Bool->Bool):BitSet {
		// var left:BitSet = this;
    ...
    static inline function combine(left:BitSet, right:BitSet, combiner:Bool->Bool->Bool):BitSet {
        var length = Ints.max(left.length, right.length);
        var acc:BitSet = new BitSet(length);

        for (index in 0...acc.length) {
            acc.setAt(index, combiner(left.at(index), right.at(index)));
        }
        return acc;
    }
    ...
    static inline function range(n: Int):Array<Int> {
        return [for (idx in (0...n)) idx];
    }
}
